...
class DatatableController extends Controller
{
    ...
    {
        $vessels = Vessel::where([['done', '1'],['archive', '1']])->orderBy('vessel_id', 'DESC')->get();
    ...
    {
        if($_GET['datetimes'] == ''){
            $loading1 = DB::table('loading')->where('vessel_id', '=', $id)->orderBy('date', 'ASC')->first();
            $date1 = $loading1->date;
            $loading2 = DB::table('loading')->where('vessel_id', '=', $id)->orderBy('date', 'DESC')->first();
            $date2 = $loading2->date;
        }else{
            $date = explode(" - ",$_GET['datetimes']);
            $date1= $date[0];
            $date2 = $date[1];   
        }
        ...
        $loading = DB::table('loading')
            ...
            ->whereBetween('loading.date', [$date1, $date2])
            ->orderByDesc('date')
            ->get();
    ...
    public function arrival($id)
    {
        ...
        foreach ($arrival as $arrive) {
            $arrive->hla1 = $arrive->hla1.' - ' .$arrive->hla2;
        }
    ...
    public function minus($id)
    {
        $minus = DB::table('minus')
            ...
            ->select('minus.*','minus.cause as car_no', 'cars.car_no as car_no')
            ->where('minus.vessel_id', '=', $id)
            ->where('cars.vessel_id', '=', $id)
            ->get();
     foreach ($minus as $min) {
            $min->car_no = $min->car_no;
        }
